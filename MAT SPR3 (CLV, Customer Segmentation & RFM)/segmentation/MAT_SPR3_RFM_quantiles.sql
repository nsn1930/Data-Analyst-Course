...
SELECT
  rTable.*,
  monetary_quantiles.percentiles[
OFFSET
  (25)] AS m25,
  monetary_quantiles.percentiles[
OFFSET
...
OFFSET
  (100)] AS m100,
  frequency_quantiles.percentiles[
OFFSET
  (25)] AS f25,
  frequency_quantiles.percentiles[
OFFSET
...
FROM
  ...
  FROM
    r_table) AS monetary_quantiles,
  (
  SELECT
    APPROX_QUANTILES(frequency, 100) percentiles
  FROM
    r_table) AS frequency_quantiles,
  (
  SELECT
    APPROX_QUANTILES(recency, 100) percentiles
  FROM
    r_table) AS recency_quantiles ),
